...
if($action == 'SAVE_COMMENT')
{
    ...
    Debug::writeToFile(array('test1' => $text), '', 'CrmCommentTime.txt');
	if($text === '')
	{
    ...
    $statusLead = '';
	if($ownerTypeID == 1){
	    $leadGet = CCrmLead::GetList(array(), array('ID' => $ownerID), array('STATUS_ID'))->Fetch();
	    $statusLead = $leadGet['STATUS_ID'];
        Debug::writeToFile(array('status' => $statusLead), '', 'CrmCommentTime.txt');
    }

	if (!\Bitrix\Crm\Security\EntityAuthorization::checkUpdatePermission($ownerTypeID, $ownerID))
	{
        if(empty($statusLead) || !in_array($statusLead, array('IN_PROCESS', 'PROCESSED', 'CONVERTED'))){
    ...
    Debug::writeToFile(array('test2' => $authorID), '', 'CrmCommentTime.txt');
    ...
    Debug::writeToFile(array('test3' => array(
        'TEXT' => $text,
        'FILES' => $attachments,
        'SETTINGS' => $settings,
        'AUTHOR_ID' => $authorID,
